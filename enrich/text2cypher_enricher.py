...
logger = logging.getLogger("real_text2cypher_enricher")
...
class RealText2CypherEnricher:
    ...
    def enrich_from_pdf(self, pdf_path):
        ...
        """
        logger.info(f"Starting knowledge graph enrichment from PDF: {pdf_path}")
        ...
        for i, chunk in enumerate(text_chunks):
            ...
            queries = self.generate_cypher_from_text(chunk)
            all_queries.extend(queries)
        
        if not all_queries:
            return {"status": "error", "message": "No Cypher queries generated"}
        
        logger.info(f"Generated {len(all_queries)} total Cypher queries")
        
        # Execute the queries
        execution_results = self.execute_cypher_queries(all_queries)
        
        return {
            "status": "success",
            "message": f"Processed {len(text_chunks)} text chunks and generated {len(all_queries)} queries",
...
if __name__ == "__main__":
    ...
    enricher = RealText2CypherEnricher(schema_file if os.path.exists(schema_file) else None)
